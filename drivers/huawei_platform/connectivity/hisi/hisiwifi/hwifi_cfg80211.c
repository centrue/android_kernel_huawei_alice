...
uint32 band_5g_enabled = FALSE;  /* TBD 5Gʹ�ܿ��أ���ǰĬ�Ͽ���������ʱ��Ҫ�޸�ΪĬ�Ϲر� */
/* DTS2014070401143 Hi110x bug fix  luolingzhi/l00225940 2014/7/22  begin */
uint8 g_ap_coex_2040m_enabled = 0;    /*ap ģʽ�·���֤�汾Ĭ�� 20M 40m����رգ���֤�汾Ĭ�Ͽ��� */
...
void wl_set_pwrm_late(struct work_struct * work)
{
    ...
    /* DTS2014012203891   Hi110x bug fix ������ 2014/01/23 begin */

    /* ���Android�߼�ѡ��ѡ��:1��ʼ�ձ��ֹ��� 2�����޳��ʱ(���״̬) */
    ...
    /* DTS2014012203891   Hi110x bug fix ������ 2014/01/23 end */
}
                                        ...
                                        struct cfg80211_scan_request *request)
{
       ...
       && (IS_CONNECTED(cfg)))
    {
        ...
        */
        ...
        /* ע��:���ܷ���-EBUSY,-EBUSY���ϲ�wpa_supplicant�����Ǵ���ɨ����,Ӱ���߼��ж� */
        return -EPERM;
...
void hwifi_get_max_rate(struct cfg_struct *cfg, struct scanned_bss_info* bss_info, struct rate_info *rate_info)
{
    ...
    if( ht_ie )/*���֧��ht*/
    {
        ...
        rate_info->legacy = 10*65 ; /*�ں˸���mcs��flag����11n������*/
        ht_cap = WPA_GET_LE16(ht_ie->element_data);
				
        if((ht_cap & (u16)(1 << 1)) && cfg->sta_info.fc_enabled)/*bit1:֧��40M����*/
        {
            rate_info->flags |= RATE_INFO_FLAGS_40_MHZ_WIDTH;
        }
        if(ht_cap & (u16)(1 << 5) )/*bit5:֧��short GI*/
        {
                                            ...
                                            struct cfg80211_connect_params *sme)
{
	...
	/* עconnect_finished�����������ط�����false�����ϸ����ں˵�ַ�䶯����һ�� */
...
#ifdef __ROAM__
/* memset cfg->conn.bss ǰ�ͷ�bss�ṹ���Ա�ж�̬����ڴ� */
    if(NULL != cfg->conn.bss.mgmt)
    {
        /* ɾ���ɵ�AP ��Ϣ */
        kfree(cfg->conn.bss.mgmt);
    ...
    if (SUCC != ret)
    {
         ...
         */
        /* DTS2013111106142 Hi110x bug fix l00225940 2013/11/13 */
        /* DTS2013120502748   Hi110x bug fix ������/hwx160629 2013/12/10 begin */
        ret = hwifi_regdomain_update(cfg);
        /* DTS2013120502748   Hi110x bug fix ������/hwx160629 2013/12/10 end */
        if (ret != SUCC)
        {
                                                ...
                                                struct station_info  *sinfo)
{
    ...
    if (IS_AP(cfg))
    {
        ...
        {
            HWIFI_INFO("get_station find_by_mac success assoc_req.ie_len=%u",
        ...
        if ((NULL != sta) && (0 != sta->assoc_req.ie_len))
        {
            ...
            HWIFI_INFO("get_station sinfo->filled=0x%llx assoc_req_ies_len=%zu",
...
static int32 wpa_parse_wpa_ie_wpa(const uint8 *ie, int32 ie_len, struct hwifi_beacon_param_stru *data)
{
    ...
    /* posǰ��8���ֽڣ���ϢԪ��id 1���ֽڣ�����1���ֽڣ�oui 4���ֽڣ�version 2���ֽ�*/
    pos = ie + 8;
    left = ie_len - 8;
...
STATIC void hwifi_cfg80211_get_channel_info(struct cfg_struct *cfg, struct cfg80211_chan_def *chan_def, struct hwifi_beacon_param_stru  *beacon_param)
{
    ...
    channel_info:
    BIT15��0->2.4G��1->5G
    BIT9-8��0->SCN��1->SCA��3->SCB��20M���������ΪSCN��40M���ѡ��SCA��SCB��
    BIT7-0�����ŵ���
    ���������8BITS����Ϊ0����AP�ڽ���������̱�ʾ�Զ��ŵ�ѡ��
    */
    if ((cfg->ap_info.auto_channel) && (!IS_P2P_ON(cfg)))
    {
        /* auto channel ��p2p������û���Զ��ŵ����� */
                                        ...
                                        u64                        *cookie)
{
    ...
    {
        *cookie = cfg->send_action_id++;   /* cookieֵ�ϲ������Ҫ�ж��Ƿ�����εķ��͵��µ�callback */
        if (*cookie == 0)
        {
		...
		/* Ϊ����P2P Action���ķ����󣬶�ʱ���������ϳ�ʱ�����޷����նԶ˵�Action���ĵ�����
		 * ÿ�η������Action���ĺ󣬸��¼�����ʱ��Ϊ500���룬�Ա����㹻ʱ��ȴ��Զ˻�ӦAction���� */
...
/* Start of zhanghua 00209041 2013-03-23 B295 ����UDP�������Ż�ά�� */
...
/*****************************************************************************
 �� �� ��  : wlan_set_and_enable_perform_debugging
 ��������  : ���ò��Զ�ʹ������ά��
 �������  : uint8 *buf
 �������  : ��
 �� �� ֵ  : void
...
void wlan_set_and_enable_perform_debugging(struct hi110x_device* hi110x_dev,uint8 *buf)
{
    ...
    /* Start of zhangwei 64406 2013-02-27 B295 �������д�ӡ */
    ...
    /* End of zhangwei 64406 2013-02-27 B295 �������д�ӡ */
    ...
    {
        hi110x_dev->hcc->sys_tcp_tx_ack_opt_enable = 0;/*tx_ack_numΪ0ʱ���ر��Ż�*/
    }
...
/*****************************************************************************
 �� �� ��  : wlan_disable_perform_debugging
 ��������  : �������ܵ���
 �������  : uint8 *buf
 �������  : ��
 �� �� ֵ  : void
...
#endif
/* End of zhanghua 00209041 2013-03-23 B295 ����UDP�������Ż�ά�� */
...
#define INIT_TIMER(timer, func, duration, extra_delay)    \
    ...
    /* 100���룬����ĳ�ʱʱ��sdio�����Լ�device���ŵ��л�ʱ�� */
                                       ...
                                       uint64                   *cookie)
{
    ...
    /* �˲���ΪTRUE˵����FIND�׶��·��ļ���״̬�������ǰ��GO��CLIENTģʽӦ��ֹ�·���device��
     * �����Ƿ��͹��������л���ָ���ļ����ŵ�����Ҫ�·���device��
     */
                                              ...
                                              uint64             cookie)
{
    ...
    /* cookieֵ��listen channel���ñ�־���ں���hwifi_cfg80211_remain_on_channel���� */
...
struct cfg_struct* hwifi_cfg80211_alloc(struct hi110x_device* hi110x_dev, struct device *dev)
{
    ...
    wiphy->max_remain_on_channel_duration = 5000;                 /* 5000��5��ʱ�䣬�����ʱ��ο�ҵ��ֵ */
